...
import { toCamelCase, toKebabCase, isArray, isFunction, isNumber, isObject, isString  } from './common';
...
export const isVElement = (vn: VNode) => {
...
export const getFirstComponent = (
  ...
  for (const child of children) {
    if (isVElement(child) || isComponent(child)) {
      return child;
    }
